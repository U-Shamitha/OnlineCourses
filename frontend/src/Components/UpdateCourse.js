...
const UpdateCourse = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (image == null) {
      ...
      try {
        await axios.post(`/updateCourse/${JSON.parse(localStorage.getItem('selectedCourse'))}`, formData, 
          {headers: {
    ...
    <div className="form-container">
      ...
      <form onSubmit={handleSubmit}>
        <div className="form-group">
          ...
          <label htmlFor="image" className={`custom-file-upload ${image != null ? 'green' : 'blue'}`}>
        ...
        <div className="form-group">
          ...
          <input type="number" step={1} min={1} max={12} style={{ width: '170px' }} name='duration' id="duration" value={duration} onChange={handleDurationChange} required />
        </div>
        <div className="form-group">
          <label htmlFor="description" className="label">Description:</label>
          <textarea  name='description' rows={3} cols={21} style={{resize:'horizontal'}} id="description" value={description} onChange={handleDesChange} required/>
